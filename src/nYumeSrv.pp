...
var
  ...
  ip       : string;
  port     : word;
  handle   : {$IFNDEF DELPHI}cardinal{$ELSE}TSocket{$ENDIF};
  newlog   : boolean;
  defaultfl: string;
  error404 : string;
  error403 : string;
...
begin
    aSection := TRTLCriticalSection.Create();
end;

procedure DoneCriticalSection( var aSection : TRTLCriticalSection );
begin
    FreeAndNil( aSection );
end;
...
procedure LeaveCriticalSection( aSection : TRTLCriticalSection );
begin
    aSection.Leave();
end;

function BeginThread(ThreadFunc: TThreadFunc; Parameter: Pointer): LongWord;
begin
    Result := 0;
    system.BeginThread( nil, 0, ThreadFunc, Parameter, 0, Result );
end;
...
begin
  ...
  if (Result = '') or (Result[Length(Result)] = '/') then Result := Result + defaultfl;
end;
...
begin
  ...
  pathDelim := {$IFDEF FPC}'/'{$ELSE}'\'{$ENDIF};
  ...
  try
    ...
    begin
      ...
      if (not FileExists(filedir + name)) and DirectoryExists(filedir + name) then name := name + pathDelim + defaultfl;

      addEnv('SCRIPT_NAME', name);

      if  FileExists(filedir + name) and ( pos( pathDelim + '..' + pathDelim, ExtractRelativepath( filedir, filedir + name ) ) <> 0) then
      begin
...
{$I+}
  ...
  repeat
    ...
    if (not needStopServer) and ( handle <> INVALID_SOCKET ) then
    begin
